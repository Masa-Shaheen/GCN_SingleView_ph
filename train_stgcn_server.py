...
# ── Early Stopping ────────────────────────────────────────────────────────
PATIENCE      = 20   # stop if no val_acc improvement for this many epochs
...
for d in [OUT_DIR, PLOTS_DIR, LOGS_DIR]:
...
class BZUDataset(Dataset):
    ...
    def _normalise_length(self, skel):
        ...
        new_skel = np.zeros(
            (self.target_frames, skel.shape[1], skel.shape[2]), dtype=np.float32)
        for joint in range(skel.shape[1]):
            for axis in range(skel.shape[2]):
                new_skel[:, joint, axis] = np.interp(
                    new_indices, old_indices, skel[:, joint, axis])
        return new_skel
...
class STGCN_SingleView(nn.Module):
    """
    Multi-task ST-GCN:
      - cls_head : classifies exercise type (E0–E9)   → Accuracy metric
      - reg_head : predicts quality score (1.0–5.0)   → RMSE / MAE / R² metrics
    """
    ...
    def forward(self, x):
        ...
        qua = 1.0 + 4.0 * torch.sigmoid(self.reg_head(x))  # (B, 1) in [1, 5]
        return cls, qua
...
def centre_and_scale(x):
    """
    Centre skeleton on mid-hip and normalise by torso height.
    x shape: (B, T, J, C)
    """
    hip     = (x[:, :, 1:2, :] + x[:, :, 4:5, :]) / 2.0
    x       = x - hip
...
def get_trial_split(df, train_ratio=TRAIN_RATIO, val_ratio=VAL_RATIO, random_state=42):
    ...
    if n_train < 1:
        raise ValueError(
            f'Not enough trials for the requested split ratios '
...
def plot_accuracy_rmse(history, save_dir):
    ...
    axes[0].plot(epochs, history['test_acc'],   label='Test',       color='green', linestyle='--')  # ← added
    ...
    axes[1].plot(epochs, history['test_rmse'],  label='Test',       color='green', linestyle='--')  # ← added
...
def plot_r2_mae(history, save_dir):
    ...
    axes[0].plot(epochs, history['test_r2'],  label='Test',       color='green', linestyle='--')  # ← added
    ...
    axes[1].plot(epochs, history['test_mae'],  label='Test',       color='green', linestyle='--')  # ← added
...
def plot_regression_scatter(q_true, q_pred, split_name='Test', save_dir=None):
    ...
    if save_dir:
        path = os.path.join(
            save_dir, f'regression_scatter_{split_name.lower()}.png')
...
def plot_early_stopping(history, best_epoch, save_dir):
    epochs = range(1, len(history['train_acc']) + 1)
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(epochs, history['val_acc'],   label='Val Accuracy',   color='darkorange')
    ax.plot(epochs, history['train_acc'], label='Train Accuracy', color='steelblue', alpha=0.6)
    ax.plot(epochs, history['test_acc'],  label='Test Accuracy',  color='green', linestyle='--')  # ← added
    ax.axvline(best_epoch, color='green', linestyle='--', linewidth=1.5,
               label=f'Best epoch ({best_epoch})')
    ax.set_title('Accuracy with Early Stopping marker', fontweight='bold')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Accuracy (%)')
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
    save_and_show(fig, os.path.join(save_dir, 'early_stopping_curve.png'))

print('✓ Plotting helpers defined')


# ══════════════════════════════════════════════════════════════════════════
# Cell 15 — Training Loop with Early Stopping
# ══════════════════════════════════════════════════════════════════════════
...
log.info('=' * 70)
log.info('STARTING TRAINING (single train/val/test split + early stopping)')
log.info('=' * 70)
...
# Change this block (history definition):
history = {k: [] for k in [
    'train_loss', 'val_loss', 'test_loss',
    'train_cls_loss', 'val_cls_loss',
    'train_reg_loss', 'val_reg_loss',
    'train_acc', 'val_acc', 'test_acc',          # ← test_acc added
    'train_rmse', 'val_rmse', 'test_rmse',        # ← test_rmse added
    'train_mae', 'val_mae', 'test_mae',           # ← test_mae added
    'train_r2', 'val_r2', 'test_r2',             # ← test_r2 added
]}

best_val_acc   = 0.0
# FIX: initialise best_model_wts immediately so it's never None
...
print(f'{"═"*65}')
log.info(f'train={len(train_df)} val={len(val_df)} test={len(test_df)} patience={PATIENCE}')
...
for epoch in range(1, EPOCHS + 1):
    ...
    # NOTE: test is NOT evaluated here — test set stays locked until
    #       training finishes and best weights are restored.
    scheduler.step()
    ...
    for k, v in [
        ...
        ('val_acc',       vl['accuracy']),  ('test_acc',     te['accuracy']),   # ← added
        ('train_rmse',    tr['rmse']),      ('val_rmse',     vl['rmse']),
        ('test_rmse',     te['rmse']),                                          # ← added
        ('train_mae',     tr['mae']),       ('val_mae',      vl['mae']),
        ('test_mae',      te['mae']),                                           # ← added
    ...
    if vl['accuracy'] > best_val_acc:
        ...
        torch.save(best_model_wts, os.path.join(OUT_DIR, 'best_model.pt'))
    ...
    if patience_counter >= PATIENCE:
        ...
        log.info(f'Early stopped at epoch {epoch}  best_epoch={best_epoch}')
        break
...
plot_early_stopping(history, best_epoch, PLOTS_DIR)
...
log.info(f'Val  Accuracy : {best_val_acc:.2f}%')
log.info(f'Val  RMSE     : {best_val_rmse:.4f}')
log.info(f'Val  MAE      : {best_val_mae:.4f}')
log.info(f'Val  R²       : {best_val_r2:.4f}')
...
print(f'✓ Best model  → {os.path.join(OUT_DIR, "best_model.pt")}')

log.info('✓ All done! Check /mvdlph/masa/GCN_SingleView_Results/ for outputs.')
