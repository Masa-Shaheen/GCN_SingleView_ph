...
class BZUDataset(Dataset):
    ...
    def _augment(self, skel):
        ...
        if random.random() < 0.50:
            ...
            skel  = np.roll(skel, shift, axis=0)

        # 3. Gaussian jitter
        if random.random() < 0.60:
            skel += np.random.randn(*skel.shape).astype(np.float32) * 0.01

        # 4. Y-axis rotation
        if random.random() < 0.50:
            angle = np.random.uniform(-8, 8) * np.pi / 180.0
        ...
        # 5. Scale
        if random.random() < 0.50:
            skel *= np.random.uniform(0.95, 1.05)
