...
# ── Mid-Fusion: feature dim per camera backbone ───────────────────────────
# Each camera's ST-GCN outputs this many features before the fusion point.
# You can change this (e.g. 32, 64, 128) — larger = more capacity per view.
CAM_FEATURE_DIM = 64      # ← the "16 features" you mentioned; set to 64 here

# ── Cross-View Transformer Fusion settings ────────────────────────────────
TRANSFORMER_NUM_HEADS    = 4       # must divide CAM_FEATURE_DIM
TRANSFORMER_NUM_LAYERS   = 2       # number of transformer encoder layers
TRANSFORMER_FFN_DIM      = 256     # feed-forward hidden dim inside transformer
TRANSFORMER_DROPOUT      = 0.2

# ── Exercise Filter ───────────────────────────────────────────────────────
EXCLUDED_EXERCISES = { 3,7, 9}# #E0 then E1
...
print(f'  CAM_FEATURE_DIM: {CAM_FEATURE_DIM}  (features per camera before fusion)')
print(f'  TRANSFORMER    : heads={TRANSFORMER_NUM_HEADS} layers={TRANSFORMER_NUM_LAYERS} ffn={TRANSFORMER_FFN_DIM}')
...
log = logging.getLogger("GCN-MidFusion-Transformer")
log.info("=" * 70)
log.info("ST-GCN Multi-View MID FUSION (Cross-View Transformer) | BZU Physiotherapy Dataset")
log.info(f"Cameras : {ALL_CAMERAS}  |  Epochs : {EPOCHS}  |  Patience : {PATIENCE}")
log.info(f"CAM_FEATURE_DIM : {CAM_FEATURE_DIM} features per camera")
...
class BZUMultiViewDataset(Dataset):
    ...
    def _augment_multiview(self, skels):
        """Same temporal warp applied to every camera."""
        T = skels[0].shape[0]
...
print('✓ BZUMultiViewDataset defined  '
      '(Mid Fusion — 3 separate (T,J,6) tensors per sample)')
print('✓ mid_fusion_collate defined')


# ══════════════════════════════════════════════════════════════════════════
# Cell 11 — ST-GCN backbone + MID FUSION Model (CROSS-VIEW TRANSFORMER)
#
#   Architecture overview
#   ─────────────────────
#
#   C0 skeleton (T,J,6) ──► STGCNCameraBackbone ──► feat_c0 (B, D) ─┐
#   C1 skeleton (T,J,6) ──► STGCNCameraBackbone ──► feat_c1 (B, D) ─┤  tokens
#   C2 skeleton (T,J,6) ──► STGCNCameraBackbone ──► feat_c2 (B, D) ─┘
#                                                              │
#                                          [CLS] + 3 camera tokens (B, 4, D)
#                                          + learnable camera-position embed
#                                                              │
#                                          Cross-View Transformer Encoder
#                                          (multi-head self-attention layers,
#                                           lets each camera attend to the
#                                           others — true cross-view fusion)
#                                                              │
...
#
#   Fusion point: a Transformer encoder operating over the 3 per-camera
#   feature vectors (treated as a sequence of "view tokens"), instead of
#   a simple concat + MLP. A learnable [CLS] token aggregates information
#   across views via self-attention and is used for the final prediction.
# ══════════════════════════════════════════════════════════════════════════

from collections import deque

# ── Set to True to tie backbone weights across cameras (fewer params) ─────
...
# STGCNCameraBackbone
#
#   Processes ONE camera's skeleton (B, T, J, 6) and outputs a
#   compact feature vector (B, CAM_FEATURE_DIM).
# ────────────────────────────────────────────────────────────────────────

class STGCNCameraBackbone(nn.Module):
    """
    Single-camera ST-GCN backbone.
    Input  : (B, T, J, 6)
    Output : (B, CAM_FEATURE_DIM)
    """
    def __init__(self, in_features=6, feature_dim=CAM_FEATURE_DIM, K=3, dropout=0.3):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return self.proj(x)                                # (B, CAM_FEATURE_DIM)


# ────────────────────────────────────────────────────────────────────────
# CrossViewTransformerFusion
#
#   Replaces the simple concat + MLP fusion with a Transformer encoder
#   operating over a sequence of "view tokens":
#     token 0      : learnable [CLS] aggregation token
#     token 1..3   : per-camera feature vectors (after backbone)
#
#   A learnable positional/view embedding is added to each camera token
#   so the model knows which camera each token came from.
#   Standard nn.TransformerEncoderLayer (self-attention + FFN) lets every
#   camera token attend to every other camera token (and the CLS token),
#   which is genuine cross-view fusion rather than naive concatenation.
#   The output CLS token embedding is taken as the fused representation.
# ────────────────────────────────────────────────────────────────────────

class CrossViewTransformerFusion(nn.Module):
    """
    Input  : list of N tensors, each (B, D)  — one per camera
    Output : (B, D)  — fused representation (CLS token output)
    """
    def __init__(self, feature_dim, num_views, num_heads=TRANSFORMER_NUM_HEADS,
                 num_layers=TRANSFORMER_NUM_LAYERS, ffn_dim=TRANSFORMER_FFN_DIM,
        ...
        # Learnable [CLS] fusion token, shape (1, 1, D) — broadcast over batch
        self.cls_token = nn.Parameter(torch.zeros(1, 1, feature_dim))
        nn.init.trunc_normal_(self.cls_token, std=0.02)

        # Learnable per-view (camera) positional embeddings: (1, N+1, D)
        # index 0 = CLS, indices 1..N = camera tokens
        self.view_pos_embed = nn.Parameter(torch.zeros(1, num_views + 1, feature_dim))
    ...
    def forward(self, cam_features):
        """
        cam_features : list of N tensors, each (B, D)
        returns      : (B, D)  fused CLS representation
        """
        B = cam_features[0].shape[0]

        # Stack camera tokens → (B, N, D)
        cam_tokens = torch.stack(cam_features, dim=1)

        # Prepend CLS token → (B, N+1, D)
        cls = self.cls_token.expand(B, -1, -1)            # (B, 1, D)
        tokens = torch.cat([cls, cam_tokens], dim=1)       # (B, N+1, D)

        # Add learnable view/positional embeddings
        tokens = tokens + self.view_pos_embed              # (B, N+1, D)

        # Cross-view self-attention (every token attends to every other)
        fused = self.encoder(tokens)                       # (B, N+1, D)
        fused = self.norm(fused)

        # Take the CLS token's output as the aggregated multi-view feature
        cls_out = fused[:, 0, :]                            # (B, D)
        return cls_out


# ────────────────────────────────────────────────────────────────────────
# STGCN_MidFusion  — the full model (Cross-View Transformer fusion)
# ────────────────────────────────────────────────────────────────────────

NUM_EXERCISES   = len(EXERCISE_REMAP)
FUSED_DIM       = CAM_FEATURE_DIM   # transformer output dim == per-camera dim (no concat blow-up)


class STGCN_MidFusion(nn.Module):
    """
    ST-GCN with MID FUSION of 3 camera views via a Cross-View Transformer.
    ...
    Fusion happens AFTER each camera's backbone compresses the skeleton
    sequence into a CAM_FEATURE_DIM-dimensional vector. The 3 vectors
    become tokens fed into a Transformer encoder (with a CLS token) that
    performs cross-view self-attention, replacing the old concat+MLP.
    """
    def __init__(self,
                 feature_dim    = CAM_FEATURE_DIM,
                 K              = 3,
                 dropout        = 0.3,
        ...
        if shared_backbone:
            self.backbone = STGCNCameraBackbone(
                in_features=6, feature_dim=feature_dim, K=K, dropout=dropout)
        else:
        ...
        # ── Cross-View Transformer fusion (replaces concat + MLP) ─────────
        self.cross_view_fusion = CrossViewTransformerFusion(
        ...
        # ── Regression head (takes fused CLS token + exercise embedding) ──
    ...
    def _init_head_weights(self):
        for m in self.regression_head.modules():
            if isinstance(m, nn.Linear):
                ...
                nn.init.ones_(m.weight)
                nn.init.zeros_(m.bias)
        nn.init.normal_(self.ex_embed.weight, 0, 0.01)
    ...
    def forward(self, cam_skels, exercise_id):
        ...
        for i, skel in enumerate(cam_skels):
            if self.shared_backbone:
                feat = self.backbone(skel)
            else:
                feat = self.backbones[i](skel)
            cam_features.append(feat)                   # each (B, CAM_FEATURE_DIM)

        # ── Cross-view Transformer fusion (self-attention across cameras) ─
        fused = self.cross_view_fusion(cam_features)     # (B, CAM_FEATURE_DIM)

        ex  = self.ex_embed(exercise_id)                 # (B, 32)
        h   = torch.cat([fused, ex], dim=1)              # (B, feature_dim + 32)

        out = 3.0 + 2.0 * torch.tanh(self.regression_head(h).squeeze(1))
        return out
...
_out        = _model(_dummy_cams, _dummy_ex)
assert _out.shape == (2,), f"Expected (2,), got {_out.shape}"
print(f'\n✓ Cross-View Transformer Fusion ST-GCN sanity check passed — output shape: {_out.shape}')

total_params = sum(p.numel() for p in _model.parameters() if p.requires_grad)
print(f'✓ Total trainable parameters: {total_params:,}')
print(f'✓ Shared backbone: {SHARED_BACKBONE}')
print(f'✓ CAM_FEATURE_DIM={CAM_FEATURE_DIM}  TRANSFORMER heads={TRANSFORMER_NUM_HEADS} '
      f'layers={TRANSFORMER_NUM_LAYERS} ffn={TRANSFORMER_FFN_DIM}')
del _dummy_cams, _dummy_ex, _model, _out

print(f'✓ STGCN_MidFusion defined  '
      f'(3 × STGCNCameraBackbone → [CLS]+tokens → Cross-View Transformer → Regression Head → score)')
...
def centre_and_scale_multiview(cam_skels):
    """
    Apply centre_and_scale independently to each camera tensor.
    cam_skels : list of 3 × (B, T, J, 6)
    Returns   : list of 3 × (B, T, J, 6)  normalised
    """
...
def run_epoch(model, loader, reg_fn, is_train=True, optimiser=None):
    ...
    with ctx:
        for cam_skels, qualities, exercise_ids in loader:
            ...
            preds = model(cam_skels, exercise_ids)
...
print('✓ centre_and_scale_multiview (per-camera independent) and run_epoch defined')
...
def plot_loss_curves(history, save_dir, test_loss=None):
    ...
    ax.set_title('Regression Loss (SmoothL1) — Mid Fusion (Cross-View Transformer)',
...
def plot_rmse_mae(history, save_dir, test_rmse=None, test_mae=None):
    ...
    fig.suptitle('RMSE & MAE — Train / Val / Test  (Mid Fusion, Cross-View Transformer)',
...
def plot_r2(history, save_dir, test_r2=None):
    ...
    ax.set_title('R² Score — Mid Fusion (Cross-View Transformer)', fontsize=13, fontweight='bold')
...
def plot_pcc(history, save_dir, test_pcc=None):
    ...
    ax.set_title('Pearson Correlation Coefficient — Mid Fusion (Cross-View Transformer)',
                 fontsize=13, fontweight='bold')
...
def plot_regression_scatter(q_true, q_pred, split_name='Test', save_dir=None):
    ...
    ax.set_title(f'{split_name} Set — True vs Predicted Quality (Mid Fusion, Cross-View Transformer)',
                 fontsize=13, fontweight='bold')
...
def plot_early_stop(history, stopped_epoch, best_epoch, save_dir):
    ...
    ax.set_title('MAE + Early Stopping — Mid Fusion (Cross-View Transformer)', fontsize=13, fontweight='bold')
...
# ── DataLoaders — note collate_fn=mid_fusion_collate ─────────────────────
...
log.info('=' * 70)
log.info('STARTING MID-FUSION (CROSS-VIEW TRANSFORMER) REGRESSION TRAINING')
log.info('=' * 70)
log.info(f'train={len(train_df)} val={len(val_df)} test={len(test_df)} '
         f'(multi-view samples, 3 cameras — mid fusion)')
log.info(f'CAM_FEATURE_DIM={CAM_FEATURE_DIM}  TRANSFORMER heads={TRANSFORMER_NUM_HEADS} '
         f'layers={TRANSFORMER_NUM_LAYERS}  SHARED_BACKBONE={SHARED_BACKBONE}')

print(f'\n{"═"*70}')
print(f'  Mid Fusion (Cross-View Transformer)  |  Cameras: {ALL_CAMERAS}  |  '
      f'CAM_FEATURE_DIM={CAM_FEATURE_DIM}')
print(f'  Transformer: heads={TRANSFORMER_NUM_HEADS}  layers={TRANSFORMER_NUM_LAYERS}  '
      f'ffn={TRANSFORMER_FFN_DIM}')
print(f'  Shared backbone: {SHARED_BACKBONE}')
...
def collect_predictions(loader):
    ...
    return all_true, all_pred, all_ex

all_true_q,     all_pred_q,     all_exercise_ids = collect_predictions(test_loader)
all_true_train, all_pred_train, _                = collect_predictions(train_loader)
all_true_val,   all_pred_val,   _                = collect_predictions(val_loader)
...
axes = axes.flatten()
fig.suptitle('Per-Exercise: True vs Predicted Quality — Test Set (Mid Fusion, Cross-View Transformer)',
             fontsize=14, fontweight='bold')
...
fig.suptitle('Per-Exercise Test Metrics (Mid Fusion, Cross-View Transformer)', fontsize=13, fontweight='bold')
...
print('=' * 65)
print('  TRAINING SUMMARY — ST-GCN Mid Fusion (Cross-View Transformer, 3 cameras)')
print('=' * 65)
print(f'  Cameras          : {ALL_CAMERAS}')
print(f'  Fusion type      : MID  (Cross-View Transformer, CLS-token aggregation)')
print(f'  CAM_FEATURE_DIM  : {CAM_FEATURE_DIM}  (per camera, = transformer d_model)')
print(f'  Transformer heads: {TRANSFORMER_NUM_HEADS}')
print(f'  Transformer layers: {TRANSFORMER_NUM_LAYERS}')
print(f'  Transformer FFN  : {TRANSFORMER_FFN_DIM}')
print(f'  Shared backbone  : {SHARED_BACKBONE}')
...
rows = [{'split': 'test_overall', 'exercise': 'all', 'fusion': 'mid_transformer',
         'cam_feature_dim': CAM_FEATURE_DIM,
