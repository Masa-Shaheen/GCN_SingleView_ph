...
CAMERA_ID     = None          # not used in multiview mode; kept for reference
...
REQUIRED_CAMERAS = [0, 1, 2]   # all three views must be present per sample
...
print(f'  MODE         : Early Fusion Multi-View (C0 + C1 + C2)')
...
log.info(f"Mode : Early Fusion (C0+C1+C2)  |  Epochs : {EPOCHS}  |  Patience : {PATIENCE}")
...
#
# Groups NPZ files by (exercise, person, trial_id, segment).
# Keeps only groups that have ALL 3 required camera views.
# Each row in the resulting DataFrame has columns:
#   filepath_C0, filepath_C1, filepath_C2
# ══════════════════════════════════════════════════════════════════════════

def build_index_multiview(split_name, df_csv, required_cameras=REQUIRED_CAMERAS):
    """
    Scan one pre-split folder (train | valid | test), group NPZ files by
    (exercise, person, trial_id, segment), keep only groups that have ALL
    required camera views, and merge quality labels from the CSV.

    Returns
    -------
    pd.DataFrame with one row per complete multi-view sample, columns:
        exercise, person, trial_id, trial_num, segment, trial_key, quality,
        filepath_C0, filepath_C1, filepath_C2
    """
    ...
    # ── Group by (exercise, person, trial_id, segment) ────────────────────
    ...
    # ── Keep only complete multi-view groups ──────────────────────────────
    records      = []
    n_incomplete = 0
    for key, cam_files in groups.items():
        if not all(c in cam_files for c in required_cameras):
            n_incomplete += 1
            continue

        exercise, person, trial_id, segment = key
        trial_num = int(trial_id[1:])

        # Merge quality label from CSV
        row = df_csv[
            (df_csv['exercise'] == f"E{exercise}") &
        ...
        for c in required_cameras:
            record[f'filepath_C{c}'] = cam_files[c]
        records.append(record)

    if n_incomplete:
        print(f'  ⚠️  Skipped {n_incomplete} groups with incomplete camera coverage')

    if not records:
        print(f'  ⚠️  No complete multi-view records for split="{split_name}"')
        return pd.DataFrame()
    ...
    print(f'  Complete multi-view samples : {len(df)}')
...
def remove_corrupted_mv(df, label='', required_cameras=REQUIRED_CAMERAS):
    """Drop rows where ANY of the required camera NPZ files cannot be loaded."""
    bad = []
    for idx, row in df.iterrows():
        for c in required_cameras:
            if load_skeleton(row[f'filepath_C{c}']) is None:
                ...
                break
    if bad:
        print(f'  [{label}] Removing {len(bad)} corrupted multi-view sample(s)')
...
train_df = build_index_multiview('train', df_csv)
val_df   = build_index_multiview('valid', df_csv)
test_df  = build_index_multiview('test',  df_csv)
...
# ══════════════════════════════════════════════════════════════════════════
# Cell 7.5 — Multi-View Completeness Check
# ══════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("Multi-view sample completeness per exercise")
print("=" * 60)
print(df_index.groupby('exercise').size().rename('samples').to_string())

print("\nPer-person sample counts across splits:")
for split_name, split_df in [('Train', train_df), ('Val', val_df), ('Test', test_df)]:
    persons = sorted(split_df['person'].unique())
    print(f"  {split_name:<6}: {persons}  ({len(split_df)} samples)")
...
# ══════════════════════════════════════════════════════════════════════════
# Cell 7.7 — Frame length distribution (using C0 as reference)
# ══════════════════════════════════════════════════════════════════════════
lengths = []
...
# ══════════════════════════════════════════════════════════════════════════
# Cell 9 — Visualise a sample skeleton (all 3 cameras)
# ══════════════════════════════════════════════════════════════════════════
idx  = 10
...
for cam in REQUIRED_CAMERAS:
    ...
    print(f'\nCamera C{cam}  shape: {skel.shape}')
    print(f'  X [{skel[:,:,0].min():.3f}, {skel[:,:,0].max():.3f}]')
    print(f'  Y [{skel[:,:,1].min():.3f}, {skel[:,:,1].max():.3f}]')
    print(f'  Z [{skel[:,:,2].min():.3f}, {skel[:,:,2].max():.3f}]')
    hip  = skel[:, 0, :].mean(axis=0)
    head = skel[:, 10, :].mean(axis=0)
    print(f'  Hip  mean XYZ : {hip.round(3)}')
    print(f'  Head mean XYZ : {head.round(3)}')
    print(f'  Head - Hip    : {(head - hip).round(3)}')

    plot_skeleton_3d(
        skel, frame_idx=0,
        title=f"C{cam} · {row['trial_key']} · E{row['exercise']}",
        save_path=os.path.join(PLOTS_DIR, f'sample_skeleton_C{cam}_3views.png'),
    )
    plot_skeleton_frames(
        skel, n_frames=5,
        title=f"C{cam} Motion · {row['trial_key']}",
        save_path=os.path.join(PLOTS_DIR, f'sample_skeleton_C{cam}_motion.png'),
    )

print('\n✓ Skeleton visualisation saved for all 3 cameras')


# ══════════════════════════════════════════════════════════════════════════
# Cell 9.5 — Camera Angle Check (adapted for multiview index)
# ══════════════════════════════════════════════════════════════════════════
target = df_index[
    (df_index['person']   == 'P0') &
    (df_index['exercise'] == 0)    &
...
if len(target) > 0:
    row_ref = target.iloc[0]
    print(f"\nComparing cameras for {row_ref['trial_key']} seg0:\n")
    for cam in REQUIRED_CAMERAS:
        skel = load_skeleton(row_ref[f'filepath_C{cam}'])
        if skel is not None:
            print(f"Camera C{cam}:")
    ...
    print("Reference sample P0/E0/T0/seg0 not found in df_index.")
...
# Early fusion:       concat along feature axis    → (T,17,18)
# ══════════════════════════════════════════════════════════════════════════

class BZUDataset_EarlyFusion(Dataset):
    """
    Returns (skeleton_fused, quality_score, exercise_id)
    skeleton_fused shape : (T, J, 18) = 3 views × [pos(3) + vel(3)]
    """
    ...
    def __getitem__(self, idx):
        ...
        # ── Load & process each view independently ────────────────────────
        view_tensors = []
        for c in self.required_cameras:
            ...
            # Velocity (finite difference, zero-padded at t=0)
            velocity      = np.zeros_like(skel)
            velocity[1:]  = skel[1:] - skel[:-1]

            # Stack pos + vel → (T, J, 6)
            view_tensors.append(np.concatenate([skel, velocity], axis=-1))

        # ── Early fusion: concat along feature axis → (T, J, 18) ─────────
...
print('✓ BZUDataset_EarlyFusion defined  (T × 17 × 18 = 3 views × pos+vel)')
...
print("Joint distances from Hip:")
for i, (name, d) in enumerate(zip(JOINT_NAMES, _dist)):
    print(f"  Joint {i:2d} ({name:<12}) distance={d}")
...
class STGCNBlock(nn.Module):
    ...
    One complete ST-GCN block:
      [Spatial Graph Conv K=3] + [Temporal Conv kernel=(9,1)] + [Residual]
    """
    def __init__(self, in_channels, out_channels, K=3,
...
class STGCN_Regression(nn.Module):
    ...
    For early fusion:  in_features=18  (3 views × [pos(3)+vel(3)])
    For single-view:   in_features=6   (1 view  × [pos(3)+vel(3)])

    Architecture:
      Input (B, T, J, in_features)
          ↓  Data BatchNorm
          ↓  9 × ST-GCN Block  (spatial K=3 + temporal conv)
          ↓  Global Average Pooling  → (B, 256)
          ↓  Exercise Embedding (32-dim)
          ↓  Regression Head
          ↓  Quality Score in [1, 5]
    """
    def __init__(self, in_features=18, K=3, dropout=0.5):
        super().__init__()
    ...
    def forward(self, x, exercise_id):
        """
        x           : (B, T, J, C)   C=18 for early fusion, C=6 for single-view
        exercise_id : (B,)
...
def centre_and_scale_early(x):
    """
    Normalise each of the 3 camera views independently.

    x : (B, T, J, 18)
        view 0 → channels  0: 6   [pos_C0(3) + vel_C0(3)]
        view 1 → channels  6:12   [pos_C1(3) + vel_C1(3)]
        view 2 → channels 12:18   [pos_C2(3) + vel_C2(3)]

    Per view:
        • translate: hip midpoint (joints 1 & 4) → origin
        • scale:     torso height (mean shoulder Y, joints 11 & 14) → 1.0
        • velocity:  divided by the same torso_h
    """
    views = []
    for v in range(3):
        pos = x[:, :, :, v*6     : v*6+3]   # (B, T, J, 3)
        vel = x[:, :, :, v*6+3   : v*6+6]   # (B, T, J, 3)
        ...
        views.append(torch.cat([pos, vel], dim=-1))   # (B, T, J, 6)

    return torch.cat(views, dim=-1)   # (B, T, J, 18)
...
def run_epoch(model, loader, optimiser, reg_fn, is_train=True):
    ...
    with ctx:
        for skels, qualities, exercise_ids in loader:
            skels        = centre_and_scale_early(skels.to(DEVICE))
...
print('✓ centre_and_scale_early and run_epoch defined')
...
def plot_loss_curves(history, save_dir, test_loss=None):
    ...
    ax.set_title('Regression Loss (Smooth L1) — Early Fusion Multi-View',
...
def plot_rmse_mae(history, save_dir, test_rmse=None, test_mae=None):
    ...
    fig.suptitle('RMSE & MAE — Train / Val / Test  (Early Fusion)',
...
def plot_r2(history, save_dir, test_r2=None):
    ...
    ax.set_title('R² Score — Early Fusion Multi-View',
                 fontsize=13, fontweight='bold')
...
def plot_pcc(history, save_dir, test_pcc=None):
    ...
    ax.set_title('Pearson Correlation Coefficient — Early Fusion Multi-View',
                 fontsize=13, fontweight='bold')
...
def plot_early_stop(history, stopped_epoch, best_epoch, save_dir):
    ...
    ax.set_title('MAE + Early Stopping — Early Fusion Multi-View',
                 fontsize=13, fontweight='bold')
...
# ══════════════════════════════════════════════════════════════════════════
# Cell 16 — Training Loop — Early Fusion Multi-View
# ══════════════════════════════════════════════════════════════════════════
...
# ── Model: in_features=18 for 3-view early fusion ────────────────────────
model = STGCN_Regression(in_features=18, K=3, dropout=0.5).to(DEVICE)
total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f'✓ Early-fusion ST-GCN  |  in_features=18  |  params={total_params:,}')
...
log.info('=' * 70)
log.info('STARTING TRAINING — Early Fusion Multi-View ST-GCN Regression')
log.info(f'train={len(train_df)}  val={len(val_df)}  test={len(test_df)}')
log.info(f'in_features=18 | Patience={PATIENCE} | LR={LR} | Batch={BATCH_SIZE}')
log.info('=' * 70)

print(f'\n{"═"*68}')
print(f'  EARLY FUSION MULTI-VIEW  (C0 + C1 + C2 → 18 features)')
...
# ── Restore best weights & single final test evaluation ──────────────────
...
plot_regression_scatter(all_true_q, all_pred_q,
                        split_name='Test_EarlyFusion', save_dir=PLOTS_DIR)
...
# ── Save history & predictions as NPZ ────────────────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════
# DIAGNOSTIC — Per-exercise skeleton variance & quality distribution
# ══════════════════════════════════════════════════════════════════════════
print("=" * 60)
...
axes = axes.flatten()
fig.suptitle('Per-Exercise: True vs Predicted Quality (Test Set — Early Fusion)',
             fontsize=14, fontweight='bold')
...
fig.suptitle('Per-Exercise Test Metrics — Early Fusion Multi-View',
             fontsize=13, fontweight='bold')
...
print('=' * 60)
print('  TRAINING SUMMARY — Early Fusion Multi-View ST-GCN')
print('=' * 60)
print(f'  Mode             : Early Fusion (C0 + C1 + C2, in_features=18)')
